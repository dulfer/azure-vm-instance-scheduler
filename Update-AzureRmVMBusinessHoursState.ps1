<#
Update-AzureRmVMBusinessHoursState.ps1
...
'BusinessHours' - Pattern describing the business hours, business timezone and workdays
...
'BusinessHoursSizePattern' - pattern describing desired VM state during and outside business hours, state can be either on or off, or a valid VM size
...
'BusinessHoursProcess' - boolean value indicating whether the resource should be processed by the script. When omitted, or true, the script will process this VM. When set to false it will be skipped.
...
Write-Output "Searching for all resources with Tag 'BusinessHours'"
$resources = Get-AzureRmResource -TagName "BusinessHours"  -ResourceType Microsoft.Compute/virtualMachines

foreach ($vm in $resources) {


    # store VM information for passing on to action-runbook/function
    $TargetVM = @{ "Name" = $vm.Name; "ResourceGroupName" = $vm.ResourceGroupName; "SubscriptionId" = $subscription.Subscription.Id }
    ...
    $businesshours = $vm.Tags["BusinessHours"]
    $pattern = $vm.Tags["BusinessHoursSizePattern"]
    ...
    $times = $businesshours.Split("|")[0].Split("{-}") # extracting business hours
    $start = $tzDate.AddHours([int]$times[0].Split(":")[0]).AddMinutes([int]$times[0].Split(":")[1]) # business hours start
    $stop = $tzDate.AddHours([int]$times[1].Split(":")[0]).AddMinutes([int]$times[1].Split(":")[1]) # business hours stop

    Write-Output "`tBusiness hours:  $($times[0])-$($times[1])" `
        "`tTimezone:        $timezone" `
    ...
    if ($isworkday -and ($now -gt $start -and $now -lt $stop)) {
        ...
        #Update-AzureRmVMBusinessHoursState -VM $TargetVM -DesiredState $($pattern.Split("|")[0])
    }
    ...
    if ($vm.Tags["BusinessHoursProcess"] -ne $null) {
        $processbusinesshours = $vm.Tags["BusinessHoursProcess"].ToLower()
        If ($processbusinesshours -eq "false") {
            Write-Output "`t*** Tag BusinessHoursProcess set to 'false', skipping"
            continue
        }
    }
